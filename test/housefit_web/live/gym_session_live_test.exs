defmodule HousefitWeb.GymSessionLiveTest do
  ...
  @create_attrs %{
    check_in_time: "2025-05-29T06:25:00Z",
    check_out_time: "2025-05-29T06:25:00Z",
    session_date: "2025-05-29",
    notes: "some notes"
  }
  @update_attrs %{
    check_in_time: "2025-05-30T06:25:00Z",
    check_out_time: "2025-05-30T06:25:00Z",
  ...
  describe "Index" do
    ...
    test "lists all gym_sessions", %{conn: conn, gym_session: gym_session} do
      {:ok, _index_live, html} = live(conn, ~p"/dashboard/gym_sessions")
    ...
    test "saves new gym_session", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/dashboard/gym_sessions")

      assert {:ok, form_live, _} =
               index_live
               |> element("a", "New Gym session")
               |> render_click()
               |> follow_redirect(conn, ~p"/dashboard/gym_sessions/new")
      ...
      assert {:ok, index_live, _html} =
               ...
               |> render_submit()
               |> follow_redirect(conn, ~p"/dashboard/gym_sessions")
    ...
    test "updates gym_session in listing", %{conn: conn, gym_session: gym_session} do
      ...
      assert {:ok, form_live, _html} =
               ...
               |> render_click()
               |> follow_redirect(conn, ~p"/dashboard/gym_sessions/#{gym_session}/edit")
    ...
    test "deletes gym_session in listing", %{conn: conn, gym_session: gym_session} do
      ...
      assert index_live
             |> element("#gym_sessions-#{gym_session.id} a", "Delete")
             |> render_click()
  ...
  describe "Show" do
    ...
    test "displays gym_session", %{conn: conn, gym_session: gym_session} do
      {:ok, _show_live, html} = live(conn, ~p"/dashboard/gym_sessions/#{gym_session}")
    ...
    test "updates gym_session and returns to show", %{conn: conn, gym_session: gym_session} do
      {:ok, show_live, _html} = live(conn, ~p"/dashboard/gym_sessions/#{gym_session}")
      ...
      assert {:ok, form_live, _} =
               ...
               |> follow_redirect(
                 conn,
                 ~p"/dashboard/gym_sessions/#{gym_session}/edit?return_to=show"
               )
      ...
      assert {:ok, show_live, _html} =
               ...
               |> form("#gym_session-form", gym_session: @update_attrs)
               |> render_submit()
               |> follow_redirect(conn, ~p"/dashboard/gym_sessions/#{gym_session}")
