defmodule HousefitWeb.MembershipTypeLiveTest do
  ...
  @create_attrs %{name: "some name", type: :time_based, description: "some description", duration_months: 42, session_count: 42, price: "120.5", is_active: true}
  ...
  @invalid_attrs %{name: nil, type: nil, description: nil, duration_months: nil, session_count: nil, price: nil, is_active: false}
  ...
  describe "Index" do
    ...
    test "lists all membership_types", %{conn: conn, membership_type: membership_type} do
      {:ok, _index_live, html} = live(conn, ~p"/membership_types")
    ...
    test "saves new membership_type", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/membership_types")

      assert {:ok, form_live, _} =
               index_live
               |> element("a", "New Membership type")
               |> render_click()
               |> follow_redirect(conn, ~p"/membership_types/new")
      ...
      assert {:ok, index_live, _html} =
               ...
               |> render_submit()
               |> follow_redirect(conn, ~p"/membership_types")
    ...
    test "updates membership_type in listing", %{conn: conn, membership_type: membership_type} do
      ...
      assert {:ok, form_live, _html} =
               ...
               |> render_click()
               |> follow_redirect(conn, ~p"/membership_types/#{membership_type}/edit")
    ...
    test "deletes membership_type in listing", %{conn: conn, membership_type: membership_type} do
      ...
      assert index_live |> element("#membership_types-#{membership_type.id} a", "Delete") |> render_click()
  ...
  describe "Show" do
    ...
    test "displays membership_type", %{conn: conn, membership_type: membership_type} do
      {:ok, _show_live, html} = live(conn, ~p"/membership_types/#{membership_type}")

      assert html =~ "Show Membership type"
      assert html =~ membership_type.name
    end

    test "updates membership_type and returns to show", %{conn: conn, membership_type: membership_type} do
      {:ok, show_live, _html} = live(conn, ~p"/membership_types/#{membership_type}")

      assert {:ok, form_live, _} =
               show_live
               |> element("a", "Edit")
               |> render_click()
               |> follow_redirect(conn, ~p"/membership_types/#{membership_type}/edit?return_to=show")
      ...
      assert {:ok, show_live, _html} =
               ...
               |> form("#membership_type-form", membership_type: @update_attrs)
               |> render_submit()
               |> follow_redirect(conn, ~p"/membership_types/#{membership_type}")
