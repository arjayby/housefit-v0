defmodule HousefitWeb.MemberMembershipLiveTest do
  ...
  @update_attrs %{
    ...
    end_date: "2025-05-30",
    sessions_remaining: 43,
    sessions_total: 43,
    payment_amount: "456.7",
  ...
  describe "Index" do
    ...
    test "lists all member_memberships", %{conn: conn, member_membership: member_membership} do
      {:ok, _index_live, html} = live(conn, ~p"/dashboard/member_memberships")
    ...
    test "saves new member_membership", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/dashboard/member_memberships")

      assert {:ok, form_live, _} =
               index_live
               |> element("a", "New Member membership")
               |> render_click()
               |> follow_redirect(conn, ~p"/dashboard/member_memberships/new")
      ...
      assert {:ok, index_live, _html} =
               ...
               |> render_submit()
               |> follow_redirect(conn, ~p"/dashboard/member_memberships")
    ...
    test "updates member_membership in listing", %{
      conn: conn,
      member_membership: member_membership
    } do
      {:ok, index_live, _html} = live(conn, ~p"/dashboard/member_memberships")
      ...
      assert {:ok, form_live, _html} =
               ...
               |> follow_redirect(
                 conn,
                 ~p"/dashboard/member_memberships/#{member_membership}/edit"
               )
    ...
    test "deletes member_membership in listing", %{
      conn: conn,
      member_membership: member_membership
    } do
      {:ok, index_live, _html} = live(conn, ~p"/dashboard/member_memberships")
      ...
      assert index_live
             |> element("#member_memberships-#{member_membership.id} a", "Delete")
             |> render_click()
  ...
  describe "Show" do
    ...
    test "displays member_membership", %{conn: conn, member_membership: member_membership} do
      {:ok, _show_live, html} = live(conn, ~p"/dashboard/member_memberships/#{member_membership}")

      assert html =~ "Show Member membership"
      assert html =~ member_membership.notes
    end

    test "updates member_membership and returns to show", %{
      conn: conn,
      member_membership: member_membership
    } do
      {:ok, show_live, _html} = live(conn, ~p"/dashboard/member_memberships/#{member_membership}")
      ...
      assert {:ok, form_live, _} =
               ...
               |> follow_redirect(
                 conn,
                 ~p"/dashboard/member_memberships/#{member_membership}/edit?return_to=show"
               )
      ...
      assert {:ok, show_live, _html} =
               ...
               |> form("#member_membership-form", member_membership: @update_attrs)
               |> render_submit()
               |> follow_redirect(conn, ~p"/dashboard/member_memberships/#{member_membership}")
